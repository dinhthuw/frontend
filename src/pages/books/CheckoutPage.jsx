...
const CheckoutPage = () => {
    ...
    const onSubmit = async (data) => {
        ...
        const productIds = cartItems.map(item => {
            if (!item._id) {
                console.error("Sản phẩm thiếu _id:", item);
            }
            return item?._id; // Sử dụng optional chaining để tránh lỗi nếu _id không có
        ...
        cartItems.forEach((item, index) => {
            console.log(`Sản phẩm ${index + 1}: ${item.title} - Giá: $${item.newPrice}`);
            console.log("ID sản phẩm:", item._id);
        });
